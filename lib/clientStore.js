...
export async function apiSignup({ email, password, name, uid, no_password }) {
  // Utilise authPost2 (avec Authorization) pour que le backend puisse détecter
  // un token @telegram.local existant et fusionner les deux comptes.
  const r = await authPost2("/api/auth/signup", { email, password, name, uid, no_password, initData: tgInitData() });
...
export async function apiTelegramAuth(initData) {
  ...
  if (r.ok && r.data.token) {
    setSession(r.data);
    // Persiste is_new en sessionStorage pour que page.jsx puisse décider skipCode.
    // Seulement pour les nouveaux comptes (is_new=true) ; on efface dès que le code
    // est validé (SignupGate.submitCode efface pi_pending_code → déclenche le reset).
    try {
      if (r.data.user?.is_new) sessionStorage.setItem("pi_tg_is_new", "1");
...
// Accès monitoring-only actif (code scope=monitoring, sans statut membre).
export function hasMonitoringAccess() {
  const u = getUser();
  const t = u && u.monitoring_until;
  if (!t) return false;
  const ms = Number(t) < 1e12 ? Number(t) * 1000 : Number(t);
  return ms > Date.now();
}

// Accès xstocks-only actif (code scope=xstocks, sans statut membre).
export function hasXStocksAccess() {
  const u = getUser();
  const t = u && u.xstocks_until;
  if (!t) return false;
  const ms = Number(t) < 1e12 ? Number(t) * 1000 : Number(t);
  return ms > Date.now();
}

// Accès invest-only actif (code scope=invest, 7 jours, sans générateur TG).
export function hasInvestAccess() {
  const u = getUser();
  const t = u && u.invest_until;
  if (!t) return false;
  const ms = Number(t) < 1e12 ? Number(t) * 1000 : Number(t);
  return ms > Date.now();
}
  ...
  try {
    ...
    if (res.ok) {
      mergeUser({ access_until: data.access_until, has_access: data.has_access,
                  monitoring_until: data.monitoring_until, xstocks_until: data.xstocks_until });
      return { ok: true, ...data };
    }
...
// Envoyer une demande d'accès au copy auto.
export async function apiCopyRequest() {
  try {
    const res = await fetch(`${API_BASE}/api/user/copy-request`, {
      method: "POST",
...
export async function apiAccessCode(code) {
  ...
  if (r.ok) {
    if (r.data.scope === "monitoring") {
      mergeUser({ monitoring_until: r.data.monitoring_until });
    } else if (r.data.scope === "xstocks") {
      mergeUser({ xstocks_until: r.data.xstocks_until });
    } else {
      mergeUser({ access_until: r.data.access_until, has_access: true });
    }
  }
...
export async function apiSendPassword(email) {
  const r = await authPost("/api/auth/send-password", { email });
  return { ok: r.ok };
}

// Incrémente le compteur de visites du user (1 ping par session navigateur).
export async function apiVisit() {
  const r = await authPost2("/api/user/visit", {});
  return r.ok ? { ok: true, visits: r.data.visits } : { ok: false };
}
...
export function photoUrl(mid) {
  const t = getToken();
  return `${API_BASE}/api/pole-trading/photo/${mid}?t=${encodeURIComponent(t || "")}`;
}

export async function poleTradingFeed() {
  const token = getToken();
  if (!token) return { ok: false, error: "unauthorized", items: [] };
  try {
    const res = await fetch(`${API_BASE}/api/pole-trading/feed`, {
      headers: { Authorization: `Bearer ${token}` },
    });
    const data = await res.json();
...
export const copySaveSpotKeys = (api_key, api_secret) =>
  copyPost("/api/copy/spot-keys", { api_key, api_secret });
...
// Option 2 : plans à exécuter manuellement (composition du master projetée sur un capital).
async function copyPlan(kind, capital) {
  const h = copyHeaders();
  if (!h) return { ok: false, error: "unauthorized" };
  try {
    const res = await fetch(`${API_BASE}/api/copy/${kind}?capital=${encodeURIComponent(capital || 0)}`,
...
export const copySpotPlan = (capital) => copyPlan("spot-plan", capital);
export const copyMarginPlan = (capital) => copyPlan("margin-plan", capital);
...
export async function apiToggleLike(contentType, contentId) {
  return apiToggleReaction(contentType, contentId, "like");
}

// Toggle d'une réaction : "like" | "bullish" | "bearish".
export async function apiToggleReaction(contentType, contentId, reaction = "like") {
  const t = getToken();
  if (!t) return { error: "unauthorized" };
  try {
    const res = await fetch(
      `${API_BASE}/api/likes/${contentType}/${contentId}?reaction=${encodeURIComponent(reaction)}`,
...
export async function apiGetLikes(contentType, ids) {
  const t = getToken();
  const headers = t ? { Authorization: `Bearer ${t}` } : {};
  try {
    const res = await fetch(
      `${API_BASE}/api/likes/${contentType}?ids=${ids.join(",")}`,
...
export async function apiContestCurrent() {
  const t = getToken();
  const headers = t ? { Authorization: `Bearer ${t}` } : {};
  try {
    const res = await fetch(`${API_BASE}/api/contest/current`, { headers, cache: "no-store" });
    return await res.json();
  } catch { return { ok: false }; }
}

export async function apiContestPredict(price, name, email, tgId) {
  const t = getToken();
  const headers = { "Content-Type": "application/json", ...(t ? { Authorization: `Bearer ${t}` } : {}) };
  try {
    const res = await fetch(`${API_BASE}/api/contest/predict`, {
      method: "POST", headers,
      body: JSON.stringify({ price, name, email: email || undefined, tg_id: tgId || undefined }),
    });
    return await res.json();
  } catch { return { error: "network" }; }
}

export async function apiContestWinnerAddress(address) {
  const t = getToken();
  const headers = { "Content-Type": "application/json", Authorization: `Bearer ${t}` };
  try {
    const res = await fetch(`${API_BASE}/api/contest/winner/address`, {
