...
async function privateGet(method, params = {}) {
  ...
  const ALLOWED = new Set(["Balance", "BalanceEx", "TradeBalance", "OpenPositions", "TradesHistory"]);
...
export const spotTradesHistory = (ofs = 0) =>
  privateGet("TradesHistory", ofs ? { ofs: String(ofs) } : {});

// Public — OHLC (pas d'auth). Renvoie result keyé par paire canonique.
// interval en minutes (1440 = journalier). `since` en epoch s.
export async function spotOHLC(pair, since, interval = 1440) {
  try {
    const res = await fetch(
      `${SPOT_BASE}/0/public/OHLC?pair=${encodeURIComponent(pair)}&interval=${interval}&since=${since}`,
      { cache: "no-store" }
    );
    const data = await res.json().catch(() => ({}));
    return data.result || {};
